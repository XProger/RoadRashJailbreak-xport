...
extern "C"
{
    ...
    /* Host pacing only: VSync polls input and advances every simulated tick
     * in either mode. Custom wait_vblank callbacks still run. Default enabled;
     * this setting survives psx_configure. Headless already skips host waits. */
    void psx_set_frame_pacing(sint32 enabled);
    sint32 psx_frame_pacing_enabled(void);
